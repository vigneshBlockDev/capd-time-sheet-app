    ...
    props.taskList.map((val, idx) => {
      return (
        <tr key={val.index}>
          ...
          <td>
            <input name="text" data-id={idx} required className="form-control" value={val.planned} onInput={(event) => props.select(val, 'P', event)} onChange= {(e) => props.selectPlanned(e.target.value)}></input>
          </td>
                  {/*
          <td>
            <input name="text" data-id={idx} className="form-control" value={val.actual} onInput={(event) => props.select(val, 'A', event)}></input>
                  </td> */}
